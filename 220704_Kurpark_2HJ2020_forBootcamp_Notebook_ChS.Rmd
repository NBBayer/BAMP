...
feature_eng <- function(dataset){
  ...
  df$Leistung_all <- df$Kessel_Leistung + df$BHKW_Leistung
  
  # TO BE ADDED: treat NA as 0 when summing up
...
Function external data: WIP - does not work yet!!

```{r}

#input: widemax data set, preferably with featrue engineering and already aggregated over Room Tpye
...
external_data <- function(dataset){
  ...
  #Join with heating data
  df_weather_holiday <- left_join(df_weather, y = public_holidays, by = "Date")
  #Replace NAs (no Holiday at date) with "kein Feiertag"
  df_weather_holiday$localName[is.na(heating_widemax_TFX_weather_holiday$localName)] <- "kein Feiertag"
