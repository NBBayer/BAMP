...
create_widemax_TFX_agg <- function(heating_long_agg, TFX_input){
  ...
  heating_widemax$Zeit <- fastPOSIXct(heating_widemax$datetime, required.components = 5L)
...
To be added: Function to aggregate over time (hourly data) and to aggregate over 
both time and RoomType
...
Function feature engineering: work in progress!
...
feature_eng <- function(dataset){
  ...
  # !! Occupancy ratio always causes error, has not been fixed yet!
...
feature_eng(HotelamKurpark_2.HJ2020_heating_widemax_TFX)
...
heating_widemax_TFX <- HotelamKurpark_2.HJ2020_heating_widemax
...
```{r}
heating_widemax_TFX_weather_holiday$Date <- fastDate(substr(heating_widemax_TFX_weather_holiday$Zeit, 0, 10))
...
#Export to csv
write.csv(heating_widemax_TFX_weather_holiday, "C:/Users/chris/Downloads/HotelamKurpark_2.HJ2020_heating_widemax_TFX_addData.csv")
```
...
Add month
```{r}
heating_data_rooms$month <- as.factor(month(heating_data_rooms$Zeit))
```

Adding external temperature from DWD
```{r}
...
MODELLING WITH AGGREGATED DATA SET
