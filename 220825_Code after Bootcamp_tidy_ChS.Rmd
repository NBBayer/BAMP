...
5) feature_eng(): Feature engineering: adding of weekday, weekend yes/no, month,
week, Occupancy ratio, Leistung gesamt

6) External_data(): Adding of weather data (DWD), public holidays, 

Remark: 5/6 not final yet!!
...
create_widemax_TFX_agg <- function(heating_long_agg, TFX_input){
  ...
  #Prepare each subset individually -> long to wide
  dfcs_building <- subset(dfcs_building, select = -c(Occ, T, Td, Val, Win, Room, DeltaTTd))
  #dfcs_building$X <- NULL
  #names(dfcs_building) <- c("Date", "Time", "ClIn.Build", "Text.Build")
  names(dfcs_building) <- c("Zeit", "ClIn.Build", "Text.Build")
  
  
  dfcs_room <- subset(dfcs_room, select = -c(Text, ClIn))
  #dfcs_room <- subset(dfcs_room, select = -c(Room, RoomType))
  #dfcs_room_wide <- spread(dfcs_room, key = "keylw", value = "Occ")
  ...
  dfcs_room_wide <- pivot_wider(dfcs_room, names_from = "Room", 
                                    values_from =  c("Occ", "T", "Td", "Val", "Win", "DeltaTTd"),
                                    names_sep = ".")
  
  #dfcs_room_wide$datetime <- paste(dfcs_room_wide$Date, dfcs_room_wide$Time)
  dfcs_room_wide$datetime <- dfcs_room_wide$Zeit
  ...
  dfcs_building$datetime <- dfcs_building$Zeit
...
df2 <- create_widemax_TFX_agg(agg_long, TFX_all_4)
...
Function also calculates Delta of T and Td (has to be done before changeing format
to widemax)
...
aggregate_room <- function(data_long){
  ...
  #Calculate Delta of T and Td -> heating to be done
  hd_Nov_2020_Agg$DeltaTTd <- hd_Nov_2020_Agg$Td - hd_Nov_2020_Agg$T
...
external_data <- function(dataset){
  ...
  clim$Time.y <- strptime(clim$MESS_DATUM, "%Y-%m-%d %H:%M:%S")
  clim$Time.y <- format(clim$Time.y, "%Y-%m-%d %H")
  df$Time.y <- strptime(clim$MESS_DATUM, "%Y-%m-%d %H:%M:%S")
  df$Time.y <- format(df$Time.y, "%Y-%m-%d %H")
  
  #Join with heating data
  df_weather <- df %>% left_join(y = clim, by = "Time.y")
  ...
  #5: Heating Degree Days HDD -> Code has to be adjusted
...
tst2 <- data.frame(external_data(df2))
